  ...
  return (
    <PageLayout>
      <div className="relative flex flex-wrap bg-slate-900 w-[64rem] h-[40rem] shadow-xl rounded-xl">
        {!!isLoading && <div
          id="display_loading"
          className="flex w-1/2 px-12 py-12 bg-gradient-to-r from-orange-500 to-lime-50
          text-3xl text-transparent bg-clip-text justify-center items-center animate-pulse">
          <span>Loading...</span>
        </div>}
        {!isLoading && <div
      ...
      <div className="flex w-full h-16 bg-slate-800 rounded-full shadow-xl items-center px-12 gap-x-4">
        <Link href="/" className="bg-orange-400 px-10 py-2 rounded-full">Home</Link>
        <button onClick={handleReload} className="bg-orange-400 px-10 py-2 rounded-full">Reload</button>
      </div>
    </PageLayout>
  );
}
